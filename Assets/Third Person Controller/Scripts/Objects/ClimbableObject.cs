...
namespace Opsive.ThirdPersonController
{
    ...
    public class ClimbableObject : MonoBehaviour
    {
        ...
        [SerializeField] private ClimbableType m_ClimbableType;
        [Tooltip("Used by all: can the character mount and face the opposite side of the ClimbableObject?")]
        [SerializeField] private bool m_CanReverseMount;
        ...
        [SerializeField] private Vector3 m_BottomMountOffset = new Vector3(-1, -1, -1);
        ...
        [SerializeField] private Vector3 m_TopMountOffset = new Vector3(-1, -1, -1);
        [Tooltip("Used by ladders and vines: the offset that the character should end up at after done mounting")]
        [SerializeField] private Vector3 m_TopMountCompleteOffset;
        [Tooltip("Used by ladders: the separation between ladder rungs")]
        [SerializeField] private float m_RungSeparation;
        [Tooltip("Used by ladders: the number of rungs that are unusable by the characters feet")]
        [SerializeField] private int m_UnusableTopRungs = 2;
        ...
        public void Mount(Transform characterTransform)
        {
            ...
            if (m_ClimbableType == ClimbableType.Ladder) {
                m_RungIndex = TopMount() ? (m_RungCount - m_UnusableTopRungs - 1) : 0;
            }
        }
        ...
        public bool CanDismount(MoveType moveType, bool stateChange)
        {
            ...
            if (m_ClimbableType == ClimbableType.Ladder) {
                ...
                if (stateChange) {
                    ...
                    if (moveType == MoveType.Up && m_RungIndex == (m_RungCount - m_UnusableTopRungs - 1)) {
                        return true;
                    }
                }
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            switch (m_ClimbableType) {
                case ClimbableType.Ladder:
                    ...
                    dismountOffset.Set(0, localSize.y / 2 - (m_UnusableTopRungs * m_RungSeparation), mountOffset.z);
                    sizeOffset.Set(0, -size.y / 2, size.z / 2);
                    Gizmos.DrawCube(dismountOffset - sizeOffset, size);
