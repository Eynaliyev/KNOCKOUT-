...
namespace Opsive.ThirdPersonController.Editor
{
    ...
    public class ItemTypeBuilder : EditorWindow
    {
        ...
        private ConsumableItemType m_UsableConsumableItem;
        ...
        private void OnGUI()
        {
            ...
            if (m_Type == ItemTypes.Primary) {
                m_UsableConsumableItem = EditorGUILayout.ObjectField("Consumable Item", m_UsableConsumableItem, typeof(ConsumableItemType), false) as ConsumableItemType;
                if (m_UsableConsumableItem != null) {
                    EditorGUI.indentLevel++;
            ...
            if (path.Length != 0 && Application.dataPath.Length < path.Length) {
                ...
                switch (m_Type) {
                    case ItemTypes.Primary:
                        ...
                        itemBaseType = primaryItem;
                        if (m_UsableConsumableItem != null) {
                            var usableConsumableItem = new PrimaryItemType.UseableConsumableItem(m_UsableConsumableItem, m_Capacity);
                            primaryItem.ConsumableItem = usableConsumableItem;
                        }
                        break;
                    case ItemTypes.Consumable:
                        itemBaseType = ScriptableObject.CreateInstance<Opsive.ThirdPersonController.Wrappers.ConsumableItemType>();
                        break;
                    case ItemTypes.Secondary:
                        ...
                        itemBaseType = secondaryItem;
                        break;
                    case ItemTypes.DualWield:
                        itemBaseType = ScriptableObject.CreateInstance<Opsive.ThirdPersonController.Wrappers.DualWieldItemType>();
                        break;
                }

                Random.seed = System.Environment.TickCount;
                itemBaseType.ID = Random.Range(0, int.MaxValue);

                path = string.Format("Assets/{0}", path.Substring(Application.dataPath.Length + 1));
                AssetDatabase.DeleteAsset(path);
                AssetDatabase.CreateAsset(itemBaseType, path);
                AssetDatabase.ImportAsset(path);
                Selection.activeObject = itemBaseType;
            }
        }
    }
}